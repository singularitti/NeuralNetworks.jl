...
!!! info
    The style guide is always a work in progress, and not all NeuralNetworks code
    follows the rules. When modifying NeuralNetworks, please fix the style violations
...
NeuralNetworks uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/singularitti/NeuralNetworks.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the NeuralNetworks directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to NeuralNetworks have
