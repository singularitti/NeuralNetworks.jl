...
[NeuralNetworks](https://github.com/singularitti/NeuralNetworks.jl).
...
version of NeuralNetworks, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/NeuralNetworks.jl"
```
...
our [GitHub repository](https://github.com/singularitti/NeuralNetworks.jl)
for new releases.
Once we release a new version, you can update NeuralNetworks by typing

```@repl
using Pkg
Pkg.update("NeuralNetworks")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall NeuralNetworks
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("NeuralNetworks")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall NeuralNetworks.
